...
class OrderController extends Controller
{
    ...
    public function show(int $id): JsonResponse
    {
        ...
        if (! $order) {
            return $this->notFoundResponse('Pedido n�o encontrado');
        }
    ...
    public function store(CreateOrderRequest $request): JsonResponse
    {
        ...
        try {
            ...
            if (! $order) {
                return $this->errorResponse('Erro ao criar pedido', null, 400);
            }
    ...
    public function updateStatus(int $id): JsonResponse
    {
        ...
        if (! auth()->user()->isAdmin()) {
            return $this->forbiddenResponse('Voc� n�o tem permiss�o para atualizar este pedido');
        }
        $status = request()->get('status');
        $validStatuses = ['pending', 'processing', 'shipped', 'delivered', 'cancelled'];
        if (! in_array($status, $validStatuses)) {
            return $this->errorResponse('Status inv�lido', null, 400);
        }
        $updated = $this->service->updateOrderStatus($id, $status);
        if (! $updated) {
            return $this->errorResponse('Erro ao atualizar pedido', null, 400);
        }
